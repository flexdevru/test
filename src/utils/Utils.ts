...
/*
class TransparencyHitArea implements PIXI.IHitArea {
	...
	private createTextureData = () => {
		...
		let texture: HTMLImageElement = this.source.texture.baseTexture.source as HTMLImageElement;
		let frame: PIXI.Rectangle = this.source.texture.frame;
		...
		this.pixelData = ctx.getImageData(0, 0, frame.width, frame.height).data;
	}
...
export class TextureHelper {
	...
	public static createRoundedTexture = (width: number, height: number, fill: string, strokeColor: string, strokeWidth: number, radius: number, offset: number = 0): Promise<PIXI.Texture> => {
		return PIXI.Texture.fromLoader(TextureHelper.createRoundedCanvas(width, height, fill, strokeColor, strokeWidth, radius, offset), '');
	}

	public static createRoundedTexture2 = (width: number, height: number, fill: string, strokeColor: string, strokeWidth: number, radius: number, offset: number = 0): PIXI.Texture => {
...
/*
	public static createGradientTexture = (size: PIXI.Point, colors: Array<string>, points: Array<PIXI.Point>, shadow: Shadow = null): PIXI.Texture => {
	...
	public static createFillTexture = (size: PIXI.Point, color: string, shadow: Shadow = null): PIXI.Texture => {
		let canvas: HTMLCanvasElement = TextureHelper.createFillCanvas(size, color, shadow);
		let texture: PIXI.Texture = PIXI.Texture.fromCanvas(canvas);
		return texture;
	}

	public static createFillTextureEx = (size: PIXI.Point, color: string, shadow: Shadow = null): PIXI.Texture => {
...
/*
		let pdf = new jsPDF({ orientation: canvas.width < canvas.height ? 'p' : 'l', unit: 'pt', format: [canvas.width / 1.33, canvas.height / 1.33] });
